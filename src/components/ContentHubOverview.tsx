...
function StageSegmentBar({ breakdown }: { breakdown: ContentHubStageBreakdown }) {
  ...
  return (
    <div
      className={cn("flex min-w-0 overflow-hidden", SEGMENT_BAR_HEIGHT)}
      role="group"
    ...
    >
      {segments.length > 0 ? (
          ...
          return (
            <Tooltip key={segment.status}>
              <TooltipTrigger
                render={
                  <span
                    className="group/segment relative flex h-full min-w-[4px] cursor-pointer items-center py-1 -my-1"
                  ...
                  >
                    <span
                      className={cn(
                        "block w-full transition-colors duration-150 ease-out",
                        SEGMENT_BAR_HEIGHT,
        ...
        <span className="h-full w-full bg-gray-100" aria-hidden />
      )}
    </div>
  );
}
